...
def get_cell_attributes(text, fontcolor=Const.BLACK, cellcolor=None, fontstyle="Regular", fontsize=11):
...
def getcalloptions(data, strikeprice, turnoverprice):
    ...
    option[Const.OI] = get_cell_attributes(
        text=data[Const.OI],
        cellcolor=Const.YELLOW if strikeprice <= turnoverprice else None
    )
...
def analyse_data(data, up, down, index):
    ...
    for strikeprice in Strikeprices:
        ...
        calls = getcalloptions(data=data[strikeprice][Const.CALLS],
                               strikeprice=strikeprice,
                               turnoverprice=data[Const.TURNOVER_PRICE])

        puts = getputoptions(data=data[strikeprice][Const.PUTS],
                             strikeprice=strikeprice,
                             turnoverprice=data[Const.TURNOVER_PRICE])
